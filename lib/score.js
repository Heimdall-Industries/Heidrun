...
class Score {
  ...
  async claimAtlas(autoClaim = false) {
    ...
    if (harvestInstructions.length) {
      ...
      if (transactions.length) {
        ...
        if (autoClaim) {
          await Write.sendDiscordMessage(
            `Claiming ATLAS in ${transactions.length} transaction(s).`,
          );
        }
        ...
        for (const transaction of transactions) {
          Write.printLine({
            text: `Preparing transaction ${transactionNumber}/${transactions.length}.`,
            color: Write.colors.fgYellow,
          });
          if (autoClaim) {
            await Write.sendDiscordMessage(
              `Preparing transaction ${transactionNumber}/${transactions.length}.`,
            );
          }
        ...
        if (txList.length) {
          ...
          if (balanceList.length) {
            ...
            if (autoClaim) {
              await Write.sendDiscordMessage(
                `You have successfully claimed ${totalAtlas.toFixed(2)} ATLAS.`,
              );
            }
            ...
            if (donationOptOut) {
              ...
              if (autoClaim) {
                await Write.sendDiscordMessage(
                  "You have opted out on donating to help support development.",
                );
              }
            } else {
              ...
              if (donationAmount > 0.01) {
                ...
                if (autoClaim) {
                  await Write.sendDiscordMessage(
                    `Proceeding with the donation of ${donationAmount.toFixed(
                      8,
                    )} ATLAS to help support development.`,
                  );
                }
                ...
                if (autoClaim) {
                  await Write.sendDiscordMessage(
                    `Your ATLAS donation with tx ${await this.connection.sendTransaction(
                      donationTx,
                      [this.keypair, this.keypair],
                    )} has been successful, thank you for your support!`,
                  );
  ...
  async processAutoBuy(shipToAutoBuy) {
    try {
      ...
      const dailyAutoBuyCheck =
        timeSinceLastCheck === 0 || timeSinceLastCheck > dayInMilliseconds;

      if (dailyAutoBuyCheck && !!shipToAutoBuy) {
        Write.printLine({
